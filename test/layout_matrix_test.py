import unittest
from bibliopixel.transforms import matrix


# class StripMapTest(unittest.TestCase):
class StripMapTest(object):
    def test_simple(self):
        m = matrix.StripMap(list(range(12)), 3)
    ...
    def test_transpose(self):
        m = matrix.StripMap(list(range(12)), 3, transpose=True)
        self.assertEqual(m.get(0, 0), 0)
    ...
    def test_reflect(self):
        m = matrix.StripMap(list(range(12)), 3, reflect_x=True)
    ...
    def test_serpentine(self):
        m = matrix.StripMap(list(range(12)), 3, serpentine_x=True)
        self.assertEqual(m.get(0, 0), 0)
        self.assertEqual(m.get(1, 0), 1)
        self.assertEqual(m.get(0, 1), 5)
        self.assertEqual(m.get(1, 1), 4)
